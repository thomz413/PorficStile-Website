...
export default function Cart({ isOpen, onClose, whatsappNumber }: CartProps) {
  const { cart, removeFromCart, updateCartItem, clearCart, getTotalItems, getTotalPrice, getCartSummary } = useCart();
  const { convertAndFormatPrice, currencyInfo } = useCurrency();

  const updateQuantity = (productId: string, talla: string, newCantidad: number) => {
  ...
  const getWhatsAppConfig = (): WhatsAppMessageConfig => {
    ...
    return {
      ...
      customNote: `Resumen del pedido:\n${summary}\n\nTotal: ${convertAndFormatPrice(totalPrice)}`
    };
  };
                ...
                {cart.map((item) => (
                  <div key={item.id} className="border border-border rounded-lg p-4 space-y-3">
                    ...
                    <div className="text-right text-sm font-medium">
                      Subtotal: {convertAndFormatPrice(
                        item.selectedItems.reduce((sum, selected) => 
                          sum + (selected.precioUnitario * selected.cantidad), 0
                        )
                      )}
                    </div>
                  </div>
