                ...
                <div key={item.id} className="border border-border rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">{item.categoria?.nombre}</p>
                    </div>
                    <button
                  ...
                  <div className="space-y-2">
                    {item.selectedItems.map((selected) => {
                      const tallaInfo = item.tallas.find((t) => t.talla === selected.talla);
                      const maxStock = tallaInfo?.stock || 0;

                      return (
                        <div key={selected.talla} className="flex items-center justify-between bg-muted/50 rounded p-2">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-sm">Talla {selected.talla}</span>
                            <span className="text-xs text-muted-foreground">({maxStock} disponibles)</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium">{convertAndFormatPrice(selected.precioUnitario)}</span>
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                onClick={() => updateQuantity(item.id, selected.talla, selected.cantidad - 1)}
                                disabled={selected.cantidad <= 0}
                                aria-label="Disminuir cantidad"
                              >
                                <Minus className="h-3 w-3" />
                              </button>
                              <div className="w-10 text-center text-sm font-medium">{selected.cantidad}</div>
                              <button
                                className="h-7 w-7 p-0 flex items-center justify-center rounded border border-border hover:bg-muted transition-all duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                                onClick={() => updateQuantity(item.id, selected.talla, selected.cantidad + 1)}
                  ...
                  <div className="text-right text-sm font-medium">
                    ...
                    {convertAndFormatPrice(
                      item.selectedItems.reduce((sum, selected) => sum + selected.precioUnitario * selected.cantidad, 0),
                    )}
                  </div>
                </div>
              ))}
        ...
        {cart.length > 0 && (
          <div className="border-t border-border p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-primary">{convertAndFormatPrice(getTotalPrice())}</span>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground text-center">📱 Recibirás confirmación y detalles de pago por WhatsApp</p>
            </div>
          </div>
        )}
      </aside>
