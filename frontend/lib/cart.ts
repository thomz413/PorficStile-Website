...
export function canAddToCart(product: Producto, variant: Variante | null): boolean {
...
export function createWhatsAppCheckoutURL(cart: CartItem[], phoneNumber?: string): string {
  const message = generateWhatsAppMessage(cart);
  const encodedMessage = encodeURIComponent(message);
  const cleanPhoneNumber = (phoneNumber || process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "51XXXXXXXXXX").replace(/\D/g, '');
