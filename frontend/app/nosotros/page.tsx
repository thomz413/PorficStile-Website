...
export default function NosotrosPage({ settings }: { settings: any }) {
		...
		<main className="min-h-screen bg-background text-foreground pt-16 overflow-hidden">
			...
			<section className="border-b border-borderbg py-16 md:py-24">
				<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
					<motion.div
						...
						className="max-w-2xl"
					>
						<motion.p variants={fadeInUp} className="text-xs font-medium text-primary uppercase tracking-[0.2em] mb-4">
							Nuestra empresa
						</motion.p>
			...
			<section className="bg-linear-to-r from-secondary to-secondary/90 text-white py-12 md:py-16 relative overflow-hidden">
				...
				<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
					...
					>
						{(estadisticas.length > 0 ? estadisticas : [
							{ textoArriba: "+5", textoAbajo: "Años de experiencia" },
							{ textoArriba: "+500", textoAbajo: "Clientes satisfechos" },
							{ textoArriba: "24/7", textoAbajo: "Soporte por WhatsApp" },
							{ textoArriba: "Variedad", textoAbajo: "Opciones para todos" },
						]).map((item, idx) => (
							<motion.div
								key={idx}
								variants={scaleIn}
			...
			<section className="py-16 md:py-20 lg:py-24 bg-muted/40">
				<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
					...
					>
						...
						<h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-foreground mb-4">Por qué elegirnos</h2>
					...
					>
						{[
							{ title: "Materiales duraderos", label: "Buena calidad", desc: "Seleccionamos telas y controles de producción...", color: "primary" },
							{ title: "Transparencia en precios", label: "Precios justos", desc: "Colaboramos directo con talleres...", color: "secondary" },
							{ title: "Hecho en Perú", label: "Producción local", desc: "Todas nuestras piezas se producen en talleres locales...", color: "accent" }
						].map((card, i) => (
							<motion.article
								key={i}
								variants={fadeInUp}
								whileHover={{ y: -8, transition: { duration: 0.2 } }}
								className={`relative overflow-hidden rounded-2xl border-2 border-${card.color}/20 bg-card p-6 md:p-8 shadow-sm`}
							>
								<div className="relative space-y-3">
									<p className={`text-xs font-medium uppercase tracking-[0.2em] text-${card.color}`}>{card.label}</p>
			...
			>
				<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
					<div className="flex-1 space-y-3 text-center lg:text-left">
						<h2 className="text-xl md:text-2xl lg:text-3xl font-semibold">¿Necesitas algo personalizado?</h2>
