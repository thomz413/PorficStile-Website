...
export default function ProductDetailClient({ product, settings }: Props) {
    // State initialization is fine. React will reset this automatically
    // when the 'key' changes in page.tsx
    ...
    const whatsappNumber = settings?.numeroWhatsapp ?? undefined;
    ...
    // FIX 1: LocalStorage Favorites (Only sets state if it actually is a favorite)
    useEffect(() => {
        try {
            const stored = localStorage.getItem("moda-peru-favorites");
            if (stored) {
                const favs = JSON.parse(stored) as string[];
                const isFav = favs.includes(String(product.id));
                        ...
                        {selectedItems.length > 0 && (
                            ...
                            >
                                ...
                                {selectedItems.map((it) => {
                                    ...
                                    const itemPrice = computeFinalPriceForVariant(it.variant ?? null);
