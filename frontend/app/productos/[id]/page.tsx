...
import { createProductOrderMessage, createGeneralQuestionMessage, createMultiSizeOrderMessage, WhatsAppMessageConfig } from "@/lib/whatsapp";
...
export default function ProductDetailPage() {
	...
	const [multiSizeSelection, setMultiSizeSelection] = useState<{ [size: string]: number }>({});
	...
	const handleAddToCart = () => {
		...
		if (isMultiSizeMode) {
			...
			selectedItems = Object.entries(multiSizeSelection)
				...
				.map(([size, qty]) => ({
					talla: size,
					cantidad: qty,
					precioUnitario: product.enOferta && product.precioDescuento ? product.precioDescuento : product.precio
				}));
		} else {
			// Single size mode
			if (selectedTalla) {
		...
		if (selectedItems.length === 0) {
			addToast({
				...
				message: 'Por favor selecciona una talla y cantidad antes de agregar al carrito',
				duration: 3000
			});
			return;
		}
	...
	const createWhatsAppConfig = (action: 'order' | 'question' | 'custom'): WhatsAppMessageConfig => {
		const baseConfig = {
		...
		if (isMultiSizeMode && getTotalQuantity() > 0) {
			...
			const messageType = action === 'question' ? 'general_question' : 
							   action === 'custom' ? 'custom_order' : 'product_order';
			...
			return {
				...
				size: sizeDetails,
				customNote: action !== 'question' ? `Pedido múltiple:\n${sizeDetails}` : undefined,
			};
		} else {
			// Single size order
			const messageType = action === 'question' ? 'general_question' : 
							...
							<div className="flex flex-col gap-4">
										...
										<div className="space-y-4">
											...
											{tallas.length > 1 && (
												<div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg border">
													...
													/>
													<label htmlFor="multiSizeMode" className="text-sm font-medium text-foreground cursor-pointer">
											...
											{!isMultiSizeMode ? (
												...
												<div className="space-y-3">
													<div className="text-sm text-muted-foreground">
														Selecciona las cantidades para cada talla que necesites:
													</div>
													{tallas.map((t) => {
														const quantity = multiSizeSelection[t.talla] || 0;
														const lowStock = t.stock > 0 && t.stock <= 3;
														return (
															<div key={t.talla} className="flex items-center gap-3 p-3 border rounded-lg">
																<div className="flex-1">
																	<div className="flex items-center gap-2">
																		<span className="font-black uppercase">{t.talla}</span>
																		{t.disponible ? (
																...
																{t.disponible && (
																	<div className="flex items-center gap-2">
																		<button
																			onClick={() => updateSizeQuantity(t.talla, quantity - 1)}
								...
								<div className="space-y-3 pt-6">
									...
									{isMultiSizeMode ? (
										...
										getTotalQuantity() === 0 ? (
											<div className="text-sm text-muted-foreground">Selecciona las cantidades de las tallas que necesitas para poder pedir.</div>
										) : (
											<WhatsAppCTA
